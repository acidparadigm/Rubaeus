    ...
    def handle_server_input(s)
        ...
        case s.strip
            ...
            when /^:(.+?)!(.+?)@(.+?)\sPRIVMSG\s.+\s:[\001]VERSION[\001]$/i
                ...
                send "NOTICE #{$1} :\001VERSION how about u buy me a drink qt\001"
            when /^:(.+?)!(.+?)@(.+?)\sPRIVMSG\s(.+)\s:EVAL (.+)$/i
                puts "[ EVAL #{$5} from #{$1}!#{$2}@#{$3} ]"
                send "PRIVMSG #{(($4==@nick)?$1:$4)} :#{evaluate($5)}"
	    when /^:(.+?)!(.+?)@(.+?)\sPRIVMSG\s(.+)\s:h$/i
		puts "h"
		send "PRIVMSG #{@channel} : h"
            else
                puts s
        end
    end
...
irc = IRC.new('irc.supernets.org', 6667, 'rubaeus', '#superbowl')
irc.connect()
begin
#    irc.main_loop()
     t1=Thread.new{irc.main_loop()}
     t2=Thread.new{irc.missingchatter()}
     t1.join
     t2.join
rescue Interrupt
rescue Exception => detail
